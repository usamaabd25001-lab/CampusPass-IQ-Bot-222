...
def verify_runtime_imports() -> dict[str, object]:
    ...
    try:
        ...
        verification_bot = Bot(token=settings.bot_token)
        verification_services = Services(verification_bot, settings, SecretBox(settings))
        check(hasattr(verification_services, "webapp_profile"), "WebApp profile service is missing")
        check(hasattr(verification_services, "webapp_provider"), "WebApp provider service is missing")
        check(hasattr(verification_services, "webapp_offer"), "WebApp offer service is missing")
    except Exception as exc:
        fail(f"Services container construction failed: {type(exc).__name__}: {exc}")

    try:
        from app.api.server import OfferCreateRequest, ProviderCreateRequest, StudentProfileUpdate
        ...
        ProviderCreateRequest(
            name_ar="منصة الطالب",
            name_en="Student Platform",
            description="منصة تجريبية",
            owner_telegram_id=8314084021,
        ...
        OfferCreateRequest(
            ...
            section_id=1,
            service_id=1,
            title="Gemini Advanced 30 يوم",
            description="خدمة تعليمية رقمية",
...
def main() -> None:
    ...
    check(
        (
            "python -m scripts.validate_webapp_contract" in dockerfile
            or "python scripts/validate_webapp_contract.py" in dockerfile
        ),
        "Dockerfile is not running the WebApp architecture contract gate",
    )
    check(
        (
