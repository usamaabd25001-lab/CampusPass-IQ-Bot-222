...
async def _v11_8_provider_commerce_webapp(session: AsyncSession) -> None:
    """Add provider-level service fee for the Web App commerce builder.

    The migration is additive and idempotent for long-lived PostgreSQL and
    SQLite recovery databases. Existing providers keep a zero provider-level
    fee until the owner explicitly configures one.
    """
    ...
    if "cp_providers" in tables:
        columns = await connection.run_sync(
            lambda sync_connection: {
                column["name"]
                for column in inspect(sync_connection).get_columns("cp_providers")
            }
        )
        if "default_service_fee_iqd" not in columns:
            await session.execute(
                text(
                    "ALTER TABLE cp_providers "
                    "ADD COLUMN default_service_fee_iqd INTEGER NOT NULL DEFAULT 0"
                )
            )
    release = await session.scalar(
        select(SystemSetting).where(SystemSetting.key == "operations.release_version")
    ...
    if release:
        release.value = "11.8.0-provider-commerce-webapp"
    else:
        session.add(
            SystemSetting(
                key="operations.release_version",
                value="11.8.0-provider-commerce-webapp",
                is_secret=False,
            )
        )
    await session.flush()
...
MIGRATIONS: tuple[Migration, ...] = (
    ...
    Migration(
        version="11.8.0-provider-commerce-webapp",
        description=(
            "Add the provider-level student service fee and the hardened provider/offer Web App "
            "commerce architecture without rewriting existing orders or offers"
        ),
        apply=_v11_8_provider_commerce_webapp,
    ),
