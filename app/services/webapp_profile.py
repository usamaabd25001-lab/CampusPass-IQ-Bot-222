...
class WebAppProfileService:
    ...
    async def save(
        ...
        values = {key: value.strip() for key, value in payload.as_dict().items()}
        completion = profile_completion(values)
        if not completion.complete:
            raise ValueError("جميع حقول الملف الشخصي مطلوبة")
        if len(values["full_name"]) < 5:
            raise ValueError("اكتب الاسم الكامل بصورة صحيحة")
        if len(values["phone"]) < 10 or not values["phone"].replace("+", "").isdigit():
