...
async def providers_list(
    ...
    webapp_url = (settings.public_base_url or "").rstrip("/")
    if webapp_url:
        rows.append([InlineKeyboardButton(
            text="➕ إضافة منصة",
            web_app=WebAppInfo(url=f"{webapp_url}/webapp/admin/provider"),
            style="success",
        )])
    else:
        rows.append([InlineKeyboardButton(text="➕ إضافة منصة", callback_data="admin:provider_add", style="success")])
    for p in providers:
        icon = "✅" if p.status == ProviderStatus.ACTIVE.value and p.is_active else "⏸"
        rows.append([InlineKeyboardButton(
            text=f"{icon} {p.name_ar} — {p.status}",
            callback_data=f"admin:provider:{p.id}",
            style="primary",
        )])
    rows.append([InlineKeyboardButton(text="↩️ رجوع", callback_data="admin:home")])
    await edit_or_send(
        callback.message,
        "🏢 <b>المنصات</b>\n\nإضافة المنصة الجديدة تتم عبر نموذج Web App آمن ومُدقّق.",
...
async def provider_commission(
    ...
    await message.answer(
        f"✅ تم إنشاء وتفعيل المنصة <b>{safe(provider.name_ar)}</b>.\n\n"
        "🖼 الشعار اختياري ويمكن إضافته لاحقًا من إدارة المنصة.",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="🏢 فتح المنصة", callback_data=f"admin:provider:{provider.id}", style="primary")]]
        ),
    )
...
async def provider_logo_cancel(
    ...
    await edit_or_send(
        callback.message,
        "تم إلغاء تغيير الشعار، وبقيت المنصة تعمل بالشعار السابق أو بدون شعار.",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
