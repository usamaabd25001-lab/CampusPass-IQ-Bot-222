...
async def provider_finance(
    ...
    accounting_balance = await services.finance.provider_balance(session, staff.provider_id)
    direct_model = not settings.provider_withdrawals_ready
    if direct_model:
        body = (
            "💰 <b>الأرباح والحسابات</b>\n\n"
            f"📊 صافي مستحقات المبيعات المسجلة: <b>{accounting_balance:,} د.ع</b>\n"
            "💳 نموذج الأموال الحالي: <b>الدفع المباشر للمنصة</b>\n\n"
            "المبلغ أعلاه سجل محاسبي لتتبع المبيعات والاسترجاعات؛ في نموذج الدفع المباشر "
            "لا يحتفظ CampusPass بأموال المنصة، لذلك لا تحتاج إلى طلب سحب من البوت."
        )
        markup = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="📊 طلب تقرير مالي", callback_data="provider:report", style="success")],
                [InlineKeyboardButton(text="↩️ لوحة المنصة", callback_data=f"provider:select:{staff.provider_id}")],
            ]
        )
        await edit_or_send(callback.message, body, reply_markup=markup)
        return

    if not await _require_entitlement(callback, session, services, staff, "withdrawals.request"):
        return
    markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="💸 طلب سحب", callback_data=f"withdraw:start:{staff.provider_id}", style="success")],
            [InlineKeyboardButton(text="📊 طلب تقرير مالي", callback_data="provider:report", style="primary")],
            [InlineKeyboardButton(text="↩️ لوحة المنصة", callback_data=f"provider:select:{staff.provider_id}")],
        ]
    )
    await edit_or_send(
        callback.message,
        "💰 <b>الأرباح والحسابات</b>\n\n"
        f"الرصيد المحاسبي المتاح للسحب: <b>{accounting_balance:,} د.ع</b>",
        reply_markup=markup,
    )
